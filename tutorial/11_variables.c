...
lval* builtin_def(lenv* e, lval* a) {
    ...
    for (int i = 0; i < syms->count; i++) {
        LASSERT(a, syms->cell[i]->type == LVAL_SYM, "Function 'def' cannont define non-symbol. Got %s, Expected %s.", ltype_name(syms->cell[i]->type), ltype_name(LVAL_SYM));
    }
    /* Check correct number of symbols and values */
    LASSERT(a, syms->count == a->count-1, "Function 'def' cannont define incorrect number of values to symbols. Got %i, Expected %i.", syms->count, a->count-1);
